...
- [x] Susturma bildirimi SEBEP içeriyor; içerik kaldırma AYRI, hedefe özel bir bildirim üretiyor (genel `message:updated`'a gömülü değil). *(2026-08-21, Slice D2.)*
- [x] Kullanıcı kendi mesajını silebiliyor. *(2026-08-21, Slice D2.)*
- [x] Düzenlenmiş bir mesaj sıradan bir görüntüleyene "(düzenlendi)" gösteriyor. *(2026-08-21, Slice D2.)*
...
### Slice D2 — Moderasyon sebebi/bildirimi + kendi mesajını silme + "düzenlendi" göstergesi (2026-08-21, tamamlandı)
Kapsam BİLEREK daraltıldı: milestone dosyasının "Sıralama" tablosu D2'yi "moderasyon itirazı" olarak adlandırıyor ama dilimin kendi GÖVDE metni sadece SEBEP alanı + hedefe-özel bildirim istiyor, ayrı bir itiraz kuyruğu/panel DEĞİL — bu ayrım plan modunda netleştirildi, tam itiraz akışı kapsam dışı bırakıldı (founder'ın kendi görevi, satır 76, bunu ileride Faz 2'ye çekme kararını hâlâ açık tutuyor).

Kod tabanı `Read`/`Grep`'le araştırıldı — `MutesService`/`ReportsService`/`MessagesGateway`'in notifyUserMuted'inin hedefe-özel emit deseni, `editMessage`'ın TAM iskeleti (self-delete için birebir şablon). Üç bilinçli tasarım kararı: (1) `deleteOwnMessage` `assertNotMuted` KULLANMIYOR — silme yeni içerik EKLEMİYOR, mute'un koruduğu risk yok, susturulmuş kullanıcı kendi mesajını yine de silebiliyor; (2) `editedAt` SADECE `editMessage` set ediyor, `removeMessageContent`/`deleteOwnMessage` set ETMİYOR — ikisi de zaten kendi açıklayıcı placeholder'ını taşıyor; (3) mute/içerik-kaldırma `reason`'ı ZORUNLU (opsiyonel değil) — AC "bildirim SEBEP içeriyor" diyor.

**Kullanıcının review'ında bulunan gerçek bir geriye-dönük-uyumluluk riski:** `MuteUserDto`/`RemoveContentDto`'ya `reason`'ı ZORUNLU eklemek `POST /moderation/users/:id/mute` ve `/reports/:id/remove-content`'i BOZAN bir değişiklik — production'da ÇALIŞAN bu iki endpoint'i mevcut deploy edilmiş frontend reason'sız çağırıyor, backend Render'a deploy edilip frontend Vercel'e HENÜZ deploy edilmemişse o aradaki pencerede 400 döner (Render/Vercel ayrı deploy hedefleri, atomik "aynı deploy" garantisi yok). Etki alanı küçük (tek-moderatörlü panel, founder'ın kendi kullanımı) ama sessizce göz ardı edilmedi — **founder'a NOT: bu dalı merge ettikten sonra Render (API) VE Vercel (web) deploy'larını ARKA ARKAYA yap, aralarında "sustur"/"içeriği kaldır" kullanma.**

Mevcut testlerin (mute/blocks/moderation backend e2e, messages/mutes/reports birim, moderation.spec.ts frontend) yeni zorunlu alanlara göre güncellenmesi beklenenden fazla zaman aldı — `reason` zorunlu hale gelince ValidationPipe UYGULANMAYAN dosyalarda (`apps/api`'nin e2e `TestingModule`'ünün genel davranışı) `@Body()` `undefined` dönüp `dto.reason`'da TypeError'a çarptı (moderation.e2e-spec.ts'te gerçek bir 500 olarak yakalandı, testin `.send()` hiç çağırmadığı ortaya çıktı).

Kendi test koşumda ikinci bir gerçek bug bulundu (kullanıcı review'ında değil): yeni fullstack `message-self-delete.spec.ts` testi `pageA.getByText("(düzenlendi)")`'i SAYFA GENELİNDE kontrol ediyordu — paylaşılan dev odasındaki AYNI test session'ının BAŞKA bir testinin (message-editing.spec.ts) düzenlediği mesaj da sayfada göründüğü için yanlışlıkla eşleşti. Kontrol silinen mesajın KENDİ satırına (`placeholderRowOnA.getByText(...)`) daraltılarak düzeltildi.
