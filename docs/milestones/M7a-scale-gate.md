...
- [ ] Gerçekçi bir yük testi (en az 300-500 eşzamanlı bağlantı, gerçek gecikme/bellek ölçümüyle) hatasız geçiyor, sonuçlar dokümante edildi. *(2026-08-19, Slice I ölçüm aracını + Slice J kod düzeltmesini (`Room.lastActivityAt` row-contention) tamamladı, GERÇEK yük testiyle doğruladı — 100-150 bağlantıda düzeltme+`connection_limit=30` ile neredeyse sıfır hata. AMA bu satır HÂLÂ AÇIK: `connection_limit`'in production `DATABASE_URL`'ine eklenmesi founder'ın işi (kod tarafı bitti, config tarafı bekliyor) — bkz. Plan notları + founder işleri listesi. Sessizce geçilmedi.)*
...
- [x] **Slice J — `MessagesService.sendMessage`'ın row-contention'ını düzelt (A17).** Saat tahmin edilmedi (henüz planlanmamıştı, gerçek ~5-6h). `Room.lastActivityAt` güncellemesi transaction'dan çıkarıldı + ateşle-unut + 30sn debounce (bkz. Plan notları) — GERÇEK yük testiyle doğrulandı, ama **KISMEN**: kod düzeltmesi kendi başına yeterli değildi, kalan boşluk Prisma'nın varsayılan connection pool boyutuydu. **Kod tarafı TAMAMLANDI, `connection_limit=30` production `DATABASE_URL`'ine eklenmesi founder'ın işi** (aşağıya bakın) — AC (satır 75) bu YÜZDEN hâlâ açık, kod eksik olduğu için değil.

## Tasks — founder'ın kendi eliyle yapacağı işler
- [ ] **Slice J'nin tamamlayıcısı — production `DATABASE_URL`'ine `?connection_limit=30` ekle.** Render dashboard'unda Postgres bağlantı stringine query-param olarak eklenir (Prisma'nın kendi okuduğu ayar, kod değişikliği gerekmiyor). Gerekçe/ölçüm için Plan notları'na bakın — local'de 100-150 bağlantıda neredeyse sıfır hata verdi, 500'de (aşırı senkron bir burst senaryosu) belirgin iyileşme sağladı ama tam temiz değildi. **Bir alttaki maddeyle (Render Postgres plan kapasitesi doğrulaması) BİRLİKTE yapılmalı** — 30, Postgres'in tipik `max_connections=100` tavanının altında güvenli bir marj bırakıyor ama gerçek Render planının bunu destekleyip desteklemediği doğrulanmalı.
...
### Slice J — `sendMessage` transaction yeniden tasarımı (2026-08-19, tamamlandı — AC KISMEN karşılandı)
