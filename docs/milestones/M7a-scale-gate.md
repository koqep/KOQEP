...
**Doğrulama:** `apps/api` lint/typecheck/build + 202 birim + 104 e2e testi (hepsi geçti). `apps/web` lint/typecheck/build + 67 Playwright testi (desktop 65 + mobile-375 2, hepsi geçti). Manuel tarayıcı doğrulaması (gerçek `SameSite=None;Secure` davranışı, localhost istisnası) implementasyon PR'ı merge olduktan sonra founder'ın kendi eliyle yapacağı iş olarak kalıyor — plan modunda bu netleştirilmişti.
