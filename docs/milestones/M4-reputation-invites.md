...
## Acceptance criteria
- [ ] Sending a message appends a `ReputationEvent` row (`MESSAGE_SENT`, diğer eylemler kapsam dışı — yukarıya bakın).
- [ ] Current level is computed/materialized from the event log, never stored as an authoritative mutable counter (ADR-0004).
- [ ] Leveling up grants exactly 1 new invite PER SEVİYE (bir olayda birden fazla seviye atlanırsa o kadar davet — aşağıdaki Plan notları'na bakın), starting at Level 1.
- [ ] Kazanılan davetler `GET /invites` ile listelenebiliyor — bu, mevcut kodun (ve `InviteView.tsx`'in) hiç sahip olmadığı yeni bir uç nokta, "davet kazandım ama göremiyorum" durumunu önlemek için zorunlu (Yol B: bu olmadan özellik kullanılamaz, kozmetik değil).
...
- [ ] **M4 Slice A — ReputationEvent günlüğü + seviye hesaplama + mesaj başı XP.** `ReputationEvent` tablosu + `User.totalXp`/`User.level` (cache, AYNI migration'da — bu oturumda öğrenilen ders: ilgili tüm alanlar aynı slice'ta). `ReputationService.awardXp` — `MessagesService.sendMessage`'ın zaten var olan transaction'ına eklenir (SocketRegistryService'in kurduğu "kesişen-endişe servisi" deseniyle aynı ruh). Saf fonksiyon `computeLevelFromXp(totalXp)` — replay testinin kalbi.
...
## Risks
- The exact level/XP formula is a product guess, not a validated curve — mitigation: ship a simple linear formula first and treat tuning as a post-launch iteration, not a blocker for this milestone. **2026-08-04 önerisi (Slice A'nın plan-modu turunda kesinleşecek):** 1 XP/mesaj, `level = floor(totalXp / 20)` — her 20 mesaj bir seviye. Tamamen tahmini, adlandırılmış sabitler olarak yazılacak, tuning sonraki bir iş.
