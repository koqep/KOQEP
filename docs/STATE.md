...
**M7a TAMAMEN KAPANDI (2026-08-19), M0-M6 TAMAMLANDI.** M7b (cila) sürüyor — Slice E, H2, D2, I2 TAMAMLANDI (D2'nin push'u kullanıcı onayında, I2 push'a hazır). Kalan tek dilim: D1 (rate limit, Faz 1 trafiği bekliyor) — milestone TAMAMEN KAPANMADI. **YENİ: M6b açıldı (5651 trafik logu, 18 ay, avukat cevabı) — sadece kapsam gözden geçirmesi yapıldı, kod dilimleri HENÜZ başlamadı.** Detaylar `docs/milestones/M7b-scale-polish.md` ve `docs/milestones/M6b-traffic-log-5651.md`'nin Plan notları.
...
- **2026-08-21 — M6b açıldı (5651 trafik logu, 18 ay saklama, avukat cevabı) — SADECE kapsam gözden geçirmesi, kod YAZILMADI.** Yeni `docs/milestones/M6b-traffic-log-5651.md` — proxy/IP zinciri, `TrafficLog` şeması, bütünlük (nitelikli zaman damgası mı kendi hash'imiz mi — avukata bağlı), 18 aylık otomatik silme, depolama tahmini (~5.3GB, Basic-256mb'nin 5GB limitine yakın/aşıyor). `docs/THREAT-MODEL.md` row 8, `docs/decisions/ADR-0005-data-retention-anonymize.md` Addendum, `docs/DATA-MODEL.md`, `docs/BACKLOG.md` A18, `docs/milestones/M6-launch-readiness.md` AC#2+go-live listesi güncellendi.
- Stack: NestJS (API+WS, Render) + Next.js (Vercel) + Postgres (Render Postgres) + Prisma + Resend + Sentry.

## Şu an üzerinde çalışılan
- **Görev:** M7b Slice D2'nin (`m7b/slice-d2-appeal-self-delete-edited-indicator`) VE Slice I2'nin (`m7b/slice-i2-en-ui-translation`, 9 commit) push'u kullanıcının onayında. M6b'nin dokümantasyon güncellemeleri henüz commit edilmedi (working tree'de).
- **Sonraki adım:** Kullanıcı push edip PR açtıktan sonra: D2 merge'i SONRASI Render+Vercel'i ARKA ARKAYA deploy et (yukarıdaki not). M6b'nin kod dilimleri (Slice A-G) avukatın 2 ek sorusu (anonimleştirme + port) VE founder'ın Render/Cloudflare proxy zinciri doğrulaması bekliyor — hiçbiri henüz başlamadı. D1 (rate limit) Faz 1 gerçek trafiğini bekliyor.
