...
- **Görev:** M1 merge sonrası bulunan bir güvenlik açığı kapatıldı: `DEV_INVITE_CODES`/`DEV_USER_EMAIL`/`DEV_USER_PASSWORD` git'te düz metin duruyordu VE her deploy production'a seed ediliyordu (dev-login'le aynı risk sınıfı, bkz. THREAT-MODEL satır 13). `seed.ts` artık `SEED_DEV_FIXTURES=true` olmadan bu ikisini oluşturmuyor (sadece oda her zaman oluşur), doğrulandı. Production temizliği (test hesabı + dev kullanıcı satırının silinmesi, yeni bir gerçek davet kodu elle eklenmesi) SQL şablonu olarak kullanıcıya verildi — Render Postgres konsoluna erişimim yok, kullanıcı kendisi çalıştıracak, henüz teyit gelmedi.
- **Yarım kalan:** Kullanıcının production'da SQL'i çalıştırıp teyit etmesi + bu fix'in push edilmesi.
- **Sonraki adım:** Kullanıcı prod SQL'ini çalıştırıp teyit etmeli. Sonrasında M2'ye mi geçilecek yoksa başka bir iş mi var, kullanıcının kararı. TOTP kurtarma akışını gerçek bir davetten önce şahsen dene (bkz. Tuzaklar) — kod hazır ama UX riski hâlâ geçerli.
