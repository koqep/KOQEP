...
**M7a TAMAMEN KAPANDI (2026-08-19), M0-M6 TAMAMLANDI.** M7b (cila) sürüyor — Slice E, H2, D2, I2 TAMAMLANDI (D2'nin push'u kullanıcı onayında, I2 push'a hazır). Kalan tek dilim: D1 (rate limit, Faz 1 trafiği bekliyor) — milestone TAMAMEN KAPANMADI. **YENİ: M6b (5651 trafik logu) + M6c (mesaj içeriği anonimleştirme denetimi) açıldı — ikisi de SADECE kapsam gözden geçirmesi, kod dilimleri HENÜZ başlamadı.** Detaylar `docs/milestones/M7b-scale-polish.md`, `docs/milestones/M6b-traffic-log-5651.md`, `docs/milestones/M6c-message-content-anonymization.md`'nin Plan notları.
...
- **2026-08-21 — M6b açıldı (5651 trafik logu, 18 ay saklama, avukat cevabı) — SADECE kapsam gözden geçirmesi, kod YAZILMADI.** Yeni `docs/milestones/M6b-traffic-log-5651.md` — proxy/IP zinciri, `TrafficLog` şeması, bütünlük (nitelikli zaman damgası mı kendi hash'imiz mi — avukata bağlı), 18 aylık otomatik silme, depolama tahmini (~5.3GB, Basic-256mb'nin 5GB limitine yakın/aşıyor).
- **2026-08-21 — M6c açıldı (mesaj içeriği anonimleştirme denetimi, avukatın ikinci cevabı) — SADECE kapsam gözden geçirmesi, kod YAZILMADI.** Yeni `docs/milestones/M6c-message-content-anonymization.md` — `deleteAccount()` bugün `Message.content`/`MessageEdit.previousContent`/`Report.reportedContent`'e hiç dokunmuyor, avukat "kim görebiliyor değil kişi belirlenebiliyor mu, satır bazlı" dedi. Üç dilim: manuel talep kanalı, hesap silme anında kullanıcı seçimi (varsayılan açık), otomatik regex taraması + geçmişe dönük backfill. `docs/THREAT-MODEL.md` row 8, `docs/decisions/ADR-0005-data-retention-anonymize.md` Addendum #2, `docs/BACKLOG.md` A19 güncellendi.
- Stack: NestJS (API+WS, Render) + Next.js (Vercel) + Postgres (Render Postgres) + Prisma + Resend + Sentry.

## Şu an üzerinde çalışılan
- **Görev:** M7b Slice D2'nin (`m7b/slice-d2-appeal-self-delete-edited-indicator`) VE Slice I2'nin (`m7b/slice-i2-en-ui-translation`, 9 commit) push'u kullanıcının onayında. M6b+M6c'nin dokümantasyon güncellemeleri henüz commit edilmedi (working tree'de).
- **Sonraki adım:** Kullanıcı push edip PR açtıktan sonra: D2 merge'i SONRASI Render+Vercel'i ARKA ARKAYA deploy et (yukarıdaki not). M6b'nin kod dilimleri founder'ın Render/Cloudflare proxy doğrulaması + Senaryo A/B kararını bekliyor. M6c'nin kod dilimleri (A/B/C) henüz başlamadı, kendi plan-modu turlarını bekliyor. D1 (rate limit) Faz 1 gerçek trafiğini bekliyor.
