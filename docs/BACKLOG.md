...
| A17 | **`MessagesService.sendMessage`'ın row-contention'ı** | 4 | M | V1.1 | M7a Slice I'nin (yük testi) GERÇEK 500-bağlantılık local koşumunda bulundu: `sendMessage`'ın `$transaction`'ı (`Message.create`+`Room.update({lastActivityAt})`+`ReputationService.awardXp`) aynı odaya (`CORE_ROOM_NAMES`) yazan yüzlerce eşzamanlı istek altında `Room.lastActivityAt` satırının kilidinde çakışıyor — 500 bağlantı/2000 mesajlık bir burst'te 2000 mesajın 239'u (%12) `Internal server error` ile başarısız oldu, p50 gecikme ~5.5sn'ye çıktı. Prisma `connection_limit`'ini 50'ye çıkarmak İYİLEŞTİRMEDİ, KÖTÜLEŞTİRDİ (p50 ~5.9sn) — kanıt: sorun bağlantı-havuzu boyutu değil, satır kilidi. Peak sunucu RSS sadece ~327MB — ARCHITECTURE.md'nin "ilk kırılacak yer WS bağlantı sayısı/bellek" tahmininin AKSİNE, önce kırılan yer bu write-path'teki lock contention. **Somut tetikleyici:** gerçek Faz 1/2 trafiğinde Sentry'de `MessagesService.sendMessage` kaynaklı `PrismaClientKnownRequestError`/timeout artışı gözlenirse, YA DA bir sonraki yük testi tekrar %5+ hata oranı gösterirse — `Room.lastActivityAt` güncellemesini transaction dışına/debounce edilmiş bir yola almak gibi bir tasarım gerekir (bu satırda ÇÖZÜLMEDİ, sadece ölçüldü — ADR-0003'ün "measured bottleneck" ilkesine uygun, mimari değişiklik ayrı bir plan-modu turu gerektirir). |
