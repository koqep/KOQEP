...
| A21 | **Otomatik isim/NER tabanlı PII tespiti (M6c'nin taramasına)** | 3 | L | V2 | `docs/milestones/M6c-message-content-anonymization.md`'nin otomatik yapısal-PII taraması (Slice C: e-posta/telefon, Slice D: TC kimlik) SADECE algoritmik/checksum'lı desenleri yakalıyor — bir isim geçtiğinde bunu (bağlam olmadan) tespit etmek gerçek bir NLP/NER modeli gerektiriyor, kod tabanında hiç yok. Gerçekçi seçenekler ikisi de sorunlu: bulut NLP servisine içerik göndermek (yeni bağımlılık+maliyet+gecikme+"PII tespiti için PII'yi üçüncü tarafa gönderme" ironisi) ya da sabit isim listesiyle eşleştirme (yüksek yanlış-pozitif/yanlış-negatif, "makul çaba" standardını gerçek regex'lerden daha güvenilir karşılayacağı şüpheli). **Somut tetikleyici:** manuel talep kanalına (RUNBOOK §3.8) isim/bağlamsal-ifşa temelli bir talep GERÇEKTEN gelmeye başlarsa (bugün sıfır — üç bileşenli mevcut güvenlik ağı yeterli görülüyor) YA DA platformda zaten planlı BAŞKA bir özellik için bir NLP servisi entegre edilirse (o zaman marjinal maliyet düşük). 2026-08-25, avukatın "isim de yakalanmalı" şartlı onayına yanıt olarak değerlendirildi ve BİLEREK ertelendi — M6c'nin Plan notları'nda detay. |
