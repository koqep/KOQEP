...
| A17 | **`MessagesService.sendMessage`'ın row-contention'ı** | 5 | M | **V1** | M7a Slice I'nin (yük testi) GERÇEK local koşumlarında bulundu: `sendMessage`'ın `$transaction`'ı (`Message.create`+`Room.update({lastActivityAt})`+`ReputationService.awardXp`) aynı odaya (`CORE_ROOM_NAMES`) yazan eşzamanlı istekler altında `Room.lastActivityAt` satırının kilidinde çakışıyor. 500 bağlantı/2000 mesajlık bir burst'te %12 hata (p50 ~5.5sn); Prisma `connection_limit`'ini 50'ye çıkarmak İYİLEŞTİRMEDİ, KÖTÜLEŞTİRDİ — sorun bağlantı-havuzu boyutu değil, satır kilidi. Peak sunucu RSS sadece ~327MB, bellek/bağlantı DEĞİL. **Eşik takibi (2026-08-19):** 50/100/150 bağlantıda tekrar ölçüldü — 50'de p50 gecikme ZATEN ~1.5sn, 100'de GERÇEK hatalar başlıyor (%1.25). Hata ORANI tekrar-koşumlar arası gürültülü/monoton değil (150'de %41 çıktı, muhtemelen aralıksız art arda testlerin local Postgres'te biriktirdiği baskı) ama GECİKME sinyali güvenilir ve monoton kötüleşiyor. **Sonuç: V1.1'den V1'e YÜKSELTİLDİ** — Faz 1'in kendi ölçeğinde (50 kullanıcı) bir burst senaryosunda sorunsuz olacağı garanti edilemiyor, `M7b-scale-polish.md`'ye ERTELENMEDİ. `docs/milestones/M7a-scale-gate.md`'ye **Slice J** olarak eklendi (kendi plan-modu turu gerektiriyor — `Room.lastActivityAt` güncellemesini transaction dışına/debounce edilmiş bir yola almak gibi bir tasarım; bu satırda ÇÖZÜLMEDİ, sadece ölçüldü, ADR-0003'ün "measured bottleneck" ilkesine uygun). |
