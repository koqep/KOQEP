...
| A18 | **Postgres plan kapasitesi (RAM/CPU/depolama) doğrulanmadı** | 3 | S | V1.1 | M7a-scale-gate.md'nin kendi AC'si bunu istiyordu ama Render'ın detaylı metrik paneli (RAM/CPU/depolama kullanım grafikleri) bir paket yükseltmesi gerektiriyor — founder'ın bugünkü Basic-256mb planında bu görünmüyor. `max_connections=100` AYRI olarak resmi dokümantasyondan doğrulandı (A17), bu madde SADECE RAM/CPU/depolamanın gerçek kullanım hacmine yeteceği sorusu. **Somut tetikleyici:** gerçek kullanıcı sayısı 50'yi geçince YA DA API'de yavaşlama/timeout gözlemlenirse — Render dashboard'unda paket yükseltilip metrikler kontrol edilir. Faz 1 (~50 kullanıcı) bu eşiğin altında kaldığı sürece bilerek beklemede — sessizce unutulmadı, ölçülmeden "yeterli" de denmedi. **2026-08-21 güncelleme:** `docs/milestones/M6b-traffic-log-5651.md`'nin kaba hesabı (500 kullanıcı, 18 ay trafik logu) tek başına ~5.3GB'a denk geliyor — Basic-256mb planının 5GB depolama limitine ÇOK YAKIN/AŞIYOR, SADECE trafik logu için, uygulamanın geri kalan verisi HARİÇ. Bu tetikleyiciyi "50 kullanıcı" eşiğinden BAĞIMSIZ olarak somutlaştırıyor — TrafficLog inşa edilmeden önce plan yükseltmesi kararı gerekebilir. **2026-08-22, founder kararı — KESİNLEŞTİ:** Kod tarafı 500 kullanıcı hedefine göre tasarlanacak (TrafficLog dahil, kapasite kararını BEKLEMEDEN ilerler — M6b Slice B zaten bu netlikle tamamlandı). Render Postgres planının yükseltilmesi TAMAMEN founder'ın kendi elinde/takdirinde — YAPILACAK, zamanlaması founder'a ait. **Tek somut, geri alınamaz sınır:** yükseltme, TrafficLog gerçek veri yazmaya başlamadan ÖNCE (M6b Slice C/D production'a deploy edilmeden önce) tamamlanmış olmalı — aksi halde ~5.3GB/18ay'lık trafik logu tek başına 5GB'lık depolama limitini doldurabilir. Bu founder-task olarak `docs/milestones/M6b-traffic-log-5651.md`'ye ve go-live kontrol listesine (`docs/milestones/M6-launch-readiness.md`) da eklendi. |
| A19 | **Geri bildirim `mailto:` hedefi kişisel gelen kutusu** | 2 | S | V1.1 | M7b Slice H2'de eklenen "geri bildirim" linki `ussasa155@gmail.com`'a gidiyor — kurumsal bir destek adresi yok, founder'ın kendi kararıyla bilerek kabul edilen bir kısıt (`RoomHeader.tsx`'in `FEEDBACK_EMAIL` sabiti, tek satırlık geçiş noktası olacak şekilde tasarlandı). **Somut tetikleyici:** support yükü kişisel gelen kutusunda karışmaya başlarsa YA DA kullanıcı sayısı Faz 2 (150) eşiğini geçerse — o zaman `support@koqep.com` gibi kurumsal bir adrese (muhtemelen Resend'in zaten kurulu olduğu domain üzerinden) geçilir. **2026-08-21 güncelleme:** `docs/milestones/M6c-message-content-anonymization.md`'nin Slice A'sı bu AYNI adresi, hesap silme sonrası kalan kimliklendirici içerik için de bir manuel talep/takedown kanalı olarak kullanıyor — kurumsal adrese geçiş kararı gelirse, bu ikinci kullanım da birlikte taşınmalı. |
...
| A21 | **Otomatik isim/NER tabanlı PII tespiti (M6c'nin taramasına) — AVUKAT ONAYLADI, KAPANDI** | 3 | L | V2 | `docs/milestones/M6c-message-content-anonymization.md`'nin otomatik yapısal-PII taraması (Slice C: e-posta/telefon) SADECE algoritmik/checksum'lı desenleri yakalıyor — bir isim geçtiğinde bunu (bağlam olmadan) tespit etmek gerçek bir NLP/NER modeli gerektiriyor, kod tabanında hiç yok. Gerçekçi seçenekler ikisi de sorunlu: bulut NLP servisine içerik göndermek (yeni bağımlılık+maliyet+gecikme+"PII tespiti için PII'yi üçüncü tarafa gönderme" ironisi) ya da sabit isim listesiyle eşleştirme (yüksek yanlış-pozitif/yanlış-negatif, "makul çaba" standardını gerçek regex'lerden daha güvenilir karşılayacağı şüpheli). **SONUÇLANDI:** avukat sordu, isim taraması OLMADAN mevcut çerçevenin YETERLİ olduğunu bildirdi (A22 ile AYNI cevap, tek soruda ikisi birden netleşti) — açık soru CEVAPLANDI. Yeniden gündeme gelmesi için tek yol: manuel talep kanalına (RUNBOOK §3.8) isim/bağlamsal-ifşa temelli bir talep GERÇEKTEN gelmeye başlarsa (bugün sıfır) YA DA platformda zaten planlı BAŞKA bir özellik için bir NLP servisi entegre edilirse. |
| A22 | **TC kimlik numarası deseni M6c'nin otomatik taramasına EKLENMEDİ (bilinçli ürün kararı) — AVUKAT ONAYLADI, KAPANDI** | 2 | S | — | TC kimlik taraması bilerek atlandı — manuel talep kanalı + kullanıcının hesap silme anındaki kendi seçimi bu riski kısmen karşılıyor, tam otomatik tarama şimdilik gerekli görülmedi. A21'den FARKI: TC kimlik YAPISAL bir desen (kontrol basamağı algoritmasıyla doğrulanabilir, `content-redaction.util.ts`'e eklenmesi ucuz/kolay olurdu — maliyet L değil S) ve kapsamı TAM olarak belirlenmişti (`docs/milestones/M6c-message-content-anonymization.md`'nin eski Slice D'si) — bu bir teknik/maliyet engeli değil, doğrudan bir ürün tercihi. **SONUÇLANDI:** avukat sordu, isim/TC kimlik taraması OLMADAN mevcut çerçevenin (e-posta+telefon regex + manuel talep kanalı + kullanıcının kendi seçimi) YETERLİ olduğunu, gerekli olmadığını bildirdi — açık soru CEVAPLANDI, bu madde artık pasif bir tetikleyici DEĞİL, sadece kayıt amaçlı. Yeniden gündeme gelmesi için tek yol: manuel talep kanalına TC kimlik temelli gerçek bir talep gelmeye başlarsa (bugün sıfır). |
| A23 | **Nitelikli zaman damgası entegrasyonu (M6b Slice I)** | 3 | L | — | Resmi bir makamdan (mahkeme, BTK, savcılık vb.) somut bir trafik logu talebi/denetimi gelirse o zaman uygulanır. Mevcut `sha256Hex` tabanlı bütünlük hash'i (Slice B/C/D, zaten canlı) iç/idari amaçlar için yeterli, sadece resmi/hukuki denetimde nitelikli damgaya ihtiyaç doğabilir. Kapsam turu TAMAMLANDI (`docs/milestones/M6b-traffic-log-5651.md`'nin Plan notları — sağlayıcı karşılaştırması, batch tasarım) ama implementasyona hiç geçilmeden ERTELENDİ — sağlayıcıya bağlanmak (hatta sağlayıcıdan bağımsız bir iskelet kurmak bile) şu an gereksiz, henüz gerçek bir ihtiyaç yok. **Somut tetikleyici:** resmi bir makamdan gerçek bir talep/denetim gelmesi (bugün sıfır). |
| A24 | **M10 mockup'ındaki presence sayısı ("5 online") Faz 2'de PLACEHOLDER kalacak** | 2 | M | V1.1 | `docs/milestones/M10-ui-redesign.md`'nin Faz 1 tasarım kararlarında bulundu — mockup'ta oda başlığında online sayısı var ama presence altyapısı (`ARCHITECTURE.md`'nin "in-process, kalıcı değil" dediği, hiç yazılmamış kısmı) hiç yok. **Somut tetikleyici:** M8'in presence/online-durum dilimi başladığında gerçek implementasyona geçilir. |
| A25 | **M10 mockup'ındaki son-mesaj önizlemesi (oda listesi satırında) Faz 2'de PLACEHOLDER kalacak** | 2 | M | V1.1 | `docs/milestones/M10-ui-redesign.md`'nin Faz 1 tasarım kararlarında bulundu — yeni bir sorgu/veri gerektiriyor (her oda için son mesajın içeriğini getirmek), bugün `listRooms` bunu döndürmüyor. **Somut tetikleyici:** M10 Faz 2 sonrası ayrı bir küçük dilim olarak ELE ALINABİLİR (M8'e bağımlı değil, sadece bir backend sorgu eklentisi — istenirse Faz 2'yle birlikte de yapılabilir, zorunlu değil). |
| A26 | **M10 mockup'ındaki okunmamış sayısı rozetleri Faz 2'de PLACEHOLDER kalacak** | 2 | M | V1.1 | `docs/milestones/M10-ui-redesign.md`'nin Faz 1 tasarım kararlarında bulundu — kullanıcının kendi listesinde YOKTU, kod okunurken AYRICA bulundu: `docs/milestones/M8-social-features.md:24`'ün `ReadCursor` modeline dayanıyor, o da henüz yok. **Somut tetikleyici:** M8 Slice G (okundu durumu) başladığında gerçek implementasyona geçilir. |
| A27 | **M10 mockup'ındaki TR/EN dil değiştirici Slice B'den bilerek ÇIKARILDI** | 2 | M | V1.1 | `docs/milestones/M10-ui-redesign.md`'nin Slice B plan modunda kullanıcı kararı — arayüz zaten tamamen İngilizce'ye çevrildi (A15), gerçek çeviri altyapısı YOK (A13'ün kapsamı, henüz başlamadı). "room settings" ile AYNI erteleme deseni — mockup'ta görünen ama arkasında gerçek işlevi olmayan bir düğme, yarım bir no-op olarak eklenmedi. **Somut tetikleyici:** A13 (i18n/dil planı) başladığında gerçek implementasyona geçilir. |
